...
def get_company(company_list) -> list:
    '''
    Статметод, который получает данные о работодателях с апи по определенным параметрам.
...
def get_company_vacancies(companies_list) -> list:
    '''
    Статметод, который получает вакансии с ранее полученных данных о работодателях
    если код ответа верный, то формирует список данных по определенным ключам.
    '''
    vacs_list = []
